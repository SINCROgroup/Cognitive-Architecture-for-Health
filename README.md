...
`python main.py 5 simulation_data` : -> run the code without selecting the exercise with 5 other participants

Useful information
- The TCP connection consists in the exchange of strings containig 3D end-effector positions and the time interval (delta_t) between each message. The string messages are structured as: `X={1st end-effector x value} Y={1st end-effector y value} Z={1st end-effector z value} / X={2nd end-effector x value} Y={2nd end-effector y value} Z={2nd end-effector z value} / … (repeat for the end-effectors you want) ... ; {delta_t value} ; {exercise_ID}`.
- To let the CA receive position data of multiple participants, concatenate them in a unique string, leave the `delta_t` as second to last element and `exercise_ID` as the last element.  As a good practice, separate each participant by a `;` and each end effector of a single participant by a `/`.
